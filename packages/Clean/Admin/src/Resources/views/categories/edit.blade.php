...
@section('title', 'Editar Categoría: ' . (is_array($category->name) ? implode(' ', $category->name) : (string) $category->name))
...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200 px-6 py-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <a href="{{ route('admin.clean.categories.show', $category) }}" class="w-10 h-10 bg-gray-100 hover:bg-gray-200 rounded-lg flex items-center justify-center transition-colors duration-200">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">{{ is_array($category->name) ? implode(' ', $category->name) : (string) $category->name }} • Modificar información de la categoría</p>
                </div>
            </div>
    ...
    <div class="px-4 py-6">
        <div class="max-w-4xl mx-auto">
            <form action="{{ route('admin.clean.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" 
                                       id="name" 
                                       name="name" 
                                       value="{{ old('name', is_array($category->name) ? implode(' ', $category->name) : (string) $category->name) }}"
                        ...
                        <div>
                            ...
                            <textarea id="description" 
                                      ...
                                      placeholder="Descripción de la categoría, productos que incluye, etc.">{{ old('description', is_array($category->description) ? implode(' ', $category->description) : (string) $category->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="usage_area" 
                                       name="usage_area" 
                                       value="{{ old('usage_area', $category->usage_area) }}"
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="surface_type" 
                                       name="surface_type" 
                                       value="{{ old('surface_type', $category->surface_type) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       value="1"
                                       {{ old('requires_dilution', $category->requires_dilution) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       value="1"
                                       {{ old('professional_use', $category->professional_use) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                            ...
                            <label for="image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p class="mb-2 text-sm text-gray-500">
                                        <span class="font-semibold">Click para {{ $category->image ? 'cambiar' : 'subir' }}</span> 
                                        {{ $category->image ? 'imagen' : 'o arrastra y suelta' }}
                                    </p>
                ...
                <div class="bg-blue-50 rounded-lg border border-blue-200 p-4">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            <h3 class="text-sm font-medium text-blue-900 mb-1">Información de la Categoría</h3>
                            <div class="text-sm text-blue-700 space-y-1">
                                ...
                                @if($category->children && $category->children->count() > 0)
                                    <p><strong>Subcategorías:</strong> {{ $category->children->count() }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        reader.onload = function(e) {
            ...
            label.innerHTML = `
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p class="text-sm text-gray-500"><span class="font-semibold">Nueva imagen seleccionada</span></p>
